...
class SSD(models.Model):
    ...
    # Оптовые цены и минимальные количества для скидок
    small_wholesale_price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True,
                                                verbose_name="Цена за мелкий опт")
    small_wholesale_min_qty = models.PositiveIntegerField(null=True, blank=True,
                                                          verbose_name="Мин. количество для мелкого опта")
    large_wholesale_price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True,
                                                verbose_name="Цена за крупный опт")
    large_wholesale_min_qty = models.PositiveIntegerField(null=True, blank=True,
...
class Cart(models.Model):
    ...
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name="Менеджер")
...
class CartItem(models.Model):
    cart = models.ForeignKey(Cart, on_delete=models.CASCADE, related_name="items", verbose_name="Партия")
    product = models.ForeignKey(SSD, on_delete=models.PROTECT, verbose_name="Товар")
    quantity = models.PositiveIntegerField(verbose_name="Количество")
    price_at_add = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Цена на момент добавления")
